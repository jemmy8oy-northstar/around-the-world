...
- [ ] No concrete types (classes, records, structs) defined in `AroundTheWorld.Abstractions`
- [ ] Service interface methods return **interfaces**, not concrete types (`IRatio`, not `Ratio`)
- [ ] `AroundTheWorld.Abstractions.csproj` has **zero** `<ProjectReference>` entries to projects with concrete types
...
- [ ] Domain models are in `AroundTheWorld.DomainModels`, data models in `AroundTheWorld.DataModels`
