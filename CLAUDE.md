...
│   ├── AroundTheWorld.WebApi/       # Entry point: routes, DI, OpenAPI
│   ├── AroundTheWorld.Services/     # Business logic, AutoMapper profiles
│   ├── AroundTheWorld.Abstractions/ # Service interfaces + model interfaces
│   ├── AroundTheWorld.Database/     # EF Core DbContext + migrations
│   ├── AroundTheWorld.EntityModels/ # Database entities (anemic POCOs)
│   ├── AroundTheWorld.DomainModels/ # Rich business-layer objects
│   └── AroundTheWorld.DataModels/   # Request/Response DTOs
...
- [ ] No concrete types in `AroundTheWorld.Abstractions` — **interfaces only** (the enums the interfaces reference are the one allowed exception; no classes/records/DTOs)
- [ ] Service interface methods return **interfaces**, not concrete types (`IRatio`, not `Ratio`)
- [ ] `AroundTheWorld.Abstractions.csproj` has **zero** `<ProjectReference>` entries to concrete projects
...
- [ ] Domain models in `AroundTheWorld.DomainModels`, data models in `AroundTheWorld.DataModels`
