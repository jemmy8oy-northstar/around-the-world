...
function range(start, stop, step) {
	...
	var i = -1, n = Math.max(0, Math.ceil((stop - start) / step)) | 0, range$2 = new Array(n);
	while (++i < n) range$2[i] = start + i * step;
	return range$2;
}
...
function area_default(object$1) {
	areaSum$1 = new Adder();
...
function spherical(cartesian$1) {
	return [atan2(cartesian$1[1], cartesian$1[0]), asin(cartesian$1[2])];
}
function cartesian(spherical$1) {
	var lambda = spherical$1[0], phi = spherical$1[1], cosPhi = cos(phi);
	return [
		cosPhi * cos(lambda),
		cosPhi * sin(lambda),
		sin(phi)
...
function rangeContains(range$2, x) {
	return range$2[0] <= range$2[1] ? range$2[0] <= x && x <= range$2[1] : x < range$2[0] || range$2[1] < x;
}
...
function rejoin_default(segments, compareIntersection$1, startInside, interpolate, stream) {
	...
	segments.forEach(function(segment) {
		...
		var n$1, p0$1 = segment[0], p1 = segment[n$1], x;
		...
		subject.push(x = new Intersection(p0$1, segment, null, true));
		clip.push(x.o = new Intersection(p0$1, null, x, false));
...
function polygonContains_default(polygon, point) {
	...
	for (var i = 0, n = polygon.length; i < n; ++i) {
		...
		var ring, m, point0 = ring[m - 1], lambda0$3 = longitude(point0), phi0$1 = point0[1] / 2 + quarterPi, sinPhi0$2 = sin(phi0$1), cosPhi0$2 = cos(phi0$1);
		for (var j = 0; j < m; ++j, lambda0$3 = lambda1$1, sinPhi0$2 = sinPhi1, cosPhi0$2 = cosPhi1, point0 = point1) {
			var point1 = ring[j], lambda1$1 = longitude(point1), phi1$1 = point1[1] / 2 + quarterPi, sinPhi1 = sin(phi1$1), cosPhi1 = cos(phi1$1), delta = lambda1$1 - lambda0$3, sign$1 = delta >= 0 ? 1 : -1, absDelta = sign$1 * delta, antimeridian = absDelta > pi, k = sinPhi0$2 * sinPhi1;
			sum.add(atan2(k * sign$1 * sin(absDelta), cosPhi0$2 * cosPhi1 + k * cos(absDelta)));
			angle$1 += antimeridian ? delta + sign$1 * tau : delta;
	...
	return (angle$1 < -epsilon || angle$1 < epsilon && sum < -epsilon2) ^ winding & 1;
}
...
function clip_default(pointVisible, clipLine, interpolate, start) {
	return function(sink) {
		...
		function ringEnd() {
			...
			ringSink.lineEnd();
			var clean = ringSink.clean(), ringSegments = ringBuffer.result(), i, n = ringSegments.length, m, segment, point$1;
			ring.pop();
			polygon.push(ring);
			ring = null;
			if (!n) return;
			if (clean & 1) {
				...
				if ((m = segment.length - 1) > 0) {
					if (!polygonStarted) sink.polygonStart(), polygonStarted = true;
					sink.lineStart();
					for (i = 0; i < m; ++i) sink.point((point$1 = segment[i])[0], point$1[1]);
					sink.lineEnd();
				}
				return;
			}
...
function clipAntimeridianLine(stream) {
	...
	return {
		...
		point: function(lambda1$1, phi1$1) {
			var sign1 = lambda1$1 > 0 ? pi : -pi, delta = abs(lambda1$1 - lambda0$3);
			if (abs(delta - pi) < epsilon) {
				stream.point(lambda0$3, phi0$1 = (phi0$1 + phi1$1) / 2 > 0 ? halfPi : -halfPi);
				...
				if (abs(lambda0$3 - sign0) < epsilon) lambda0$3 -= sign0 * epsilon;
				if (abs(lambda1$1 - sign1) < epsilon) lambda1$1 -= sign1 * epsilon;
...
function clipAntimeridianIntersect(lambda0$3, phi0$1, lambda1$1, phi1$1) {
	var cosPhi0$2, cosPhi1, sinLambda0Lambda1 = sin(lambda0$3 - lambda1$1);
	return abs(sinLambda0Lambda1) > epsilon ? atan((sin(phi0$1) * (cosPhi1 = cos(phi1$1)) * sin(lambda1$1) - sin(phi1$1) * (cosPhi0$2 = cos(phi0$1)) * sin(lambda0$3)) / (cosPhi0$2 * cosPhi1 * sinLambda0Lambda1)) : (phi0$1 + phi1$1) / 2;
}
...
function circle_default$1(radius) {
	...
	function intersect(a, b, two) {
		...
		q = spherical(q);
		if (!two) return q;
		var lambda0$3 = a[0], lambda1$1 = b[0], phi0$1 = a[1], phi1$1 = b[1], z;
		if (lambda1$1 < lambda0$3) z = lambda0$3, lambda0$3 = lambda1$1, lambda1$1 = z;
		var delta$1 = lambda1$1 - lambda0$3, polar = abs(delta$1 - pi) < epsilon, meridian = polar || delta$1 < epsilon;
		if (!polar && phi1$1 < phi0$1) z = phi0$1, phi0$1 = phi1$1, phi1$1 = z;
		if (meridian ? polar ? phi0$1 + phi1$1 > 0 ^ q[1] < (abs(q[0] - lambda0$3) < epsilon ? phi0$1 : phi1$1) : phi0$1 <= q[1] && q[1] <= phi1$1 : delta$1 > pi ^ (lambda0$3 <= q[0] && q[0] <= lambda1$1)) {
	...
	function code(lambda, phi) {
		var r = smallRadius ? radius : pi - radius, code$1 = 0;
		if (lambda < -r) code$1 |= 1;
		else if (lambda > r) code$1 |= 2;
		if (phi < -r) code$1 |= 4;
		else if (phi > r) code$1 |= 8;
		return code$1;
	}
...
function line_default(a, b, x0$5, y0$5, x1$1, y1$1) {
	var ax = a[0], ay = a[1], bx = b[0], by = b[1], t0 = 0, t1 = 1, dx = bx - ax, dy = by - ay, r = x0$5 - ax;
	if (!dx && r > 0) return;
	r /= dx;
	if (dx < 0) {
		if (r < t0) return;
...
function clipRectangle(x0$5, y0$5, x1$1, y1$1) {
	function visible(x, y) {
		return x0$5 <= x && x <= x1$1 && y0$5 <= y && y <= y1$1;
	}
	function interpolate(from, to, direction, stream) {
		var a = 0, a1 = 0;
		if (from == null || (a = corner(from, direction)) !== (a1 = corner(to, direction)) || comparePoint(from, to) < 0 ^ direction > 0) do
			stream.point(a === 0 || a === 3 ? x0$5 : x1$1, a > 1 ? y1$1 : y0$5);
		while ((a = (a + direction + 4) % 4) !== a1);
		else stream.point(to[0], to[1]);
	}
	function corner(p, direction) {
		return abs(p[0] - x0$5) < epsilon ? direction > 0 ? 0 : 3 : abs(p[0] - x1$1) < epsilon ? direction > 0 ? 2 : 1 : abs(p[1] - y0$5) < epsilon ? direction > 0 ? 1 : 0 : direction > 0 ? 3 : 2;
	}
	...
	return function(stream) {
		...
		function polygonInside() {
			var winding = 0;
			for (var i = 0, n = polygon.length; i < n; ++i) for (var ring$1 = polygon[i], j = 1, m = ring$1.length, point$1 = ring$1[0], a0, a1, b0 = point$1[0], b1 = point$1[1]; j < m; ++j) {
				a0 = b0, a1 = b1, point$1 = ring$1[j], b0 = point$1[0], b1 = point$1[1];
				if (a1 <= y1$1) {
					if (b1 > y1$1 && (b0 - a0) * (y1$1 - a1) > (b1 - a1) * (x0$5 - a0)) ++winding;
				} else if (b1 <= y1$1 && (b0 - a0) * (y1$1 - a1) < (b1 - a1) * (x0$5 - a0)) --winding;
			}
			return winding;
		}
		...
		function linePoint$1(x, y) {
			...
			if (first) {
				...
				if (line_default(a, b, x0$5, y0$5, x1$1, y1$1)) {
					if (!v_) {
						activeStream.lineStart();
...
var containsObjectType = {
	Feature: function(object$1, point) {
		return containsGeometry(object$1.geometry, point);
	},
	FeatureCollection: function(object$1, point) {
		var features = object$1.features, i = -1, n = features.length;
...
var containsGeometryType = {
	...
	Point: function(object$1, point) {
		return containsPoint(object$1.coordinates, point);
	},
	MultiPoint: function(object$1, point) {
		var coordinates$1 = object$1.coordinates, i = -1, n = coordinates$1.length;
		while (++i < n) if (containsPoint(coordinates$1[i], point)) return true;
		return false;
	},
	LineString: function(object$1, point) {
		return containsLine(object$1.coordinates, point);
	},
	MultiLineString: function(object$1, point) {
		var coordinates$1 = object$1.coordinates, i = -1, n = coordinates$1.length;
		while (++i < n) if (containsLine(coordinates$1[i], point)) return true;
		return false;
	},
	Polygon: function(object$1, point) {
		return containsPolygon(object$1.coordinates, point);
	},
	MultiPolygon: function(object$1, point) {
		var coordinates$1 = object$1.coordinates, i = -1, n = coordinates$1.length;
		while (++i < n) if (containsPolygon(coordinates$1[i], point)) return true;
		return false;
	},
	GeometryCollection: function(object$1, point) {
		var geometries = object$1.geometries, i = -1, n = geometries.length;
...
function containsPoint(coordinates$1, point) {
	return distance_default(coordinates$1, point) === 0;
}
function containsLine(coordinates$1, point) {
	var ao, bo, ab;
	for (var i = 0, n = coordinates$1.length; i < n; i++) {
		bo = distance_default(coordinates$1[i], point);
		if (bo === 0) return true;
		if (i > 0) {
			ab = distance_default(coordinates$1[i], coordinates$1[i - 1]);
...
function containsPolygon(coordinates$1, point) {
	return !!polygonContains_default(coordinates$1.map(ringRadians), pointRadians(point));
}
...
function contains_default(object$1, point) {
	return (object$1 && containsObjectType.hasOwnProperty(object$1.type) ? containsObjectType[object$1.type] : containsGeometry)(object$1, point);
}

//#endregion
//#region node_modules/d3-geo/src/graticule.js
function graticuleX(y0$5, y1$1, dy) {
	var y = range(y0$5, y1$1 - epsilon, dy).concat(y1$1);
	return function(x) {
		return y.map(function(y$1) {
			return [x, y$1];
		});
	};
}
function graticuleY(x0$5, x1$1, dx) {
	var x = range(x0$5, x1$1 - epsilon, dx).concat(x1$1);
	return function(y) {
		return x.map(function(x$1) {
			return [x$1, y];
		});
	};
}
function graticule() {
	var x1$1, x0$5, X1$2, X0$2, y1$1, y0$5, Y1$2, Y0$2, dx = 10, dy = dx, DX = 90, DY = 360, x, y, X, Y, precision = 2.5;
	...
	function lines() {
		return range(ceil(X0$2 / DX) * DX, X1$2, DX).map(X).concat(range(ceil(Y0$2 / DY) * DY, Y1$2, DY).map(Y)).concat(range(ceil(x0$5 / dx) * dx, x1$1, dx).filter(function(x$1) {
			return abs(x$1 % DX) > epsilon;
		}).map(x)).concat(range(ceil(y0$5 / dy) * dy, y1$1, dy).filter(function(y$1) {
			return abs(y$1 % DY) > epsilon;
		}).map(y));
	}
	...
	graticule$1.outline = function() {
		return {
			type: "Polygon",
			coordinates: [X(X0$2).concat(Y(Y1$2).slice(1), X(X1$2).reverse().slice(1), Y(Y0$2).reverse().slice(1))]
		};
	};
	...
	graticule$1.precision = function(_) {
		...
		precision = +_;
		x = graticuleX(y0$5, y1$1, 90);
		y = graticuleY(x0$5, x1$1, precision);
		X = graticuleX(Y0$2, Y1$2, 90);
		Y = graticuleY(X0$2, X1$2, precision);
		return graticule$1;
	};
	return graticule$1.extentMajor([[-180, -90 + epsilon], [180, 90 - epsilon]]).extentMinor([[-180, -80 - epsilon], [180, 80 + epsilon]]);
}
...
function interpolate_default(a, b) {
	var x0$5 = a[0] * radians, y0$5 = a[1] * radians, x1$1 = b[0] * radians, y1$1 = b[1] * radians, cy0 = cos(y0$5), sy0 = sin(y0$5), cy1 = cos(y1$1), sy1 = sin(y1$1), kx0 = cy0 * cos(x0$5), ky0 = cy0 * sin(x0$5), kx1 = cy1 * cos(x1$1), ky1 = cy1 * sin(x1$1), d = 2 * asin(sqrt(haversin(y1$1 - y0$5) + cy0 * cy1 * haversin(x1$1 - x0$5))), k = sin(d);
...
function appendRound(digits) {
	...
	if (d !== cacheDigits) {
		const k = 10 ** d;
		cacheDigits = d;
		cacheAppend = function append$1(strings) {
			let i = 1;
			this._ += strings[0];
...
function path_default(projection$1, context) {
	...
	path.area = function(object$1) {
		stream_default(object$1, projectionStream(area_default$1));
		return area_default$1.result();
	};
	path.measure = function(object$1) {
		stream_default(object$1, projectionStream(measure_default));
		return measure_default.result();
	};
	path.bounds = function(object$1) {
		stream_default(object$1, projectionStream(bounds_default$1));
		return bounds_default$1.result();
	};
	path.centroid = function(object$1) {
		stream_default(object$1, projectionStream(centroid_default$1));
		return centroid_default$1.result();
	};
	path.projection = function(_) {
		if (!arguments.length) return projection$1;
		projectionStream = _ == null ? (projection$1 = null, identity_default$1) : (projection$1 = _).stream;
		return path;
	};
	...
	return path.projection(projection$1).digits(digits).context(context);
}
...
function fitExtent(projection$1, extent, object$1) {
	return fit(projection$1, function(b) {
		var w = extent[1][0] - extent[0][0], h = extent[1][1] - extent[0][1], k = Math.min(w / (b[1][0] - b[0][0]), h / (b[1][1] - b[0][1])), x = +extent[0][0] + (w - k * (b[1][0] + b[0][0])) / 2, y = +extent[0][1] + (h - k * (b[1][1] + b[0][1])) / 2;
		projection$1.scale(150 * k).translate([x, y]);
	}, object$1);
}
...
function resample(project, delta2) {
	function resampleLineTo(x0$5, y0$5, lambda0$3, a0, b0, c0, x1$1, y1$1, lambda1$1, a1, b1, c1, depth, stream) {
		var dx = x1$1 - x0$5, dy = y1$1 - y0$5, d2 = dx * dx + dy * dy;
		if (d2 > 4 * delta2 && depth--) {
			var a = a0 + a1, b = b0 + b1, c = c0 + c1, m = sqrt(a * a + b * b + c * c), phi2 = asin(c /= m), lambda2$1 = abs(abs(c) - 1) < epsilon || abs(lambda0$3 - lambda1$1) < epsilon ? (lambda0$3 + lambda1$1) / 2 : atan2(b, a), p = project(lambda2$1, phi2), x2 = p[0], y2 = p[1], dx2 = x2 - x0$5, dy2 = y2 - y0$5, dz = dy * dx2 - dx * dy2;
			if (dz * dz / d2 > delta2 || abs((dx * dx2 + dy * dy2) / d2 - .5) > .3 || a0 * a1 + b0 * b1 + c0 * c1 < cosMinDistance) {
				resampleLineTo(x0$5, y0$5, lambda0$3, a0, b0, c0, x2, y2, lambda2$1, a /= m, b /= m, c, depth, stream);
				stream.point(x2, y2);
				resampleLineTo(x2, y2, lambda2$1, a, b, c, x1$1, y1$1, lambda1$1, a1, b1, c1, depth, stream);
			}
		}
	}
	return function(stream) {
		var lambda00$3, x00$3, y00$3, a00, b00, c00, lambda0$3, x0$5, y0$5, a0, b0, c0;
		var resampleStream = {
			point,
			lineStart,
			lineEnd,
		...
		function linePoint$1(lambda, phi) {
			...
			resampleLineTo(x0$5, y0$5, lambda0$3, a0, b0, c0, x0$5 = p[0], y0$5 = p[1], lambda0$3 = lambda, a0 = c[0], b0 = c[1], c0 = c[2], maxDepth, stream);
		...
		function ringPoint(lambda, phi) {
			linePoint$1(lambda00$3 = lambda, phi), x00$3 = x0$5, y00$3 = y0$5, a00 = a0, b00 = b0, c00 = c0;
			resampleStream.point = linePoint$1;
		}
		function ringEnd() {
			resampleLineTo(x0$5, y0$5, lambda0$3, a0, b0, c0, x00$3, y00$3, lambda00$3, a00, b00, c00, maxDepth, stream);
...
function projectionMutator(projectAt) {
	var project, k = 150, x = 480, y = 250, lambda = 0, phi = 0, deltaLambda = 0, deltaPhi = 0, deltaGamma = 0, rotate, alpha = 0, sx = 1, sy = 1, theta = null, preclip = antimeridian_default, x0$5 = null, y0$5, x1$1, y1$1, postclip = identity_default$1, delta2 = .5, projectResample, projectTransform, projectRotateTransform, cache, cacheStream;
	...
	projection$1.clipExtent = function(_) {
		return arguments.length ? (postclip = _ == null ? (x0$5 = y0$5 = x1$1 = y1$1 = null, identity_default$1) : clipRectangle(x0$5 = +_[0][0], y0$5 = +_[0][1], x1$1 = +_[1][0], y1$1 = +_[1][1]), reset()) : x0$5 == null ? null : [[x0$5, y0$5], [x1$1, y1$1]];
	};
	...
	projection$1.fitExtent = function(extent, object$1) {
		return fitExtent(projection$1, extent, object$1);
	};
...
function conicProjection(projectAt) {
	var phi0$1 = 0, phi1$1 = pi / 3, m = projectionMutator(projectAt), p = m(phi0$1, phi1$1);
	p.parallels = function(_) {
		return arguments.length ? m(phi0$1 = _[0] * radians, phi1$1 = _[1] * radians) : [phi0$1 * degrees, phi1$1 * degrees];
	};
	return p;
}
...
function cylindricalEqualAreaRaw(phi0$1) {
	var cosPhi0$2 = cos(phi0$1);
	function forward(lambda, phi) {
		return [lambda * cosPhi0$2, sin(phi) / cosPhi0$2];
	}
	forward.invert = function(x, y) {
		return [x / cosPhi0$2, asin(y * cosPhi0$2)];
	};
	return forward;
}
...
function conicEqualAreaRaw(y0$5, y1$1) {
	var sy0 = sin(y0$5), n = (sy0 + sin(y1$1)) / 2;
	if (abs(n) < epsilon) return cylindricalEqualAreaRaw(y0$5);
...
function albersUsa_default() {
	...
	function albersUsa(coordinates$1) {
		var x = coordinates$1[0], y = coordinates$1[1];
		return point = null, (lower48Point.point(x, y), point) || (alaskaPoint.point(x, y), point) || (hawaiiPoint.point(x, y), point);
	}
	albersUsa.invert = function(coordinates$1) {
		var k = lower48.scale(), t = lower48.translate(), x = (coordinates$1[0] - t[0]) / k, y = (coordinates$1[1] - t[1]) / k;
		return (y >= .12 && y < .234 && x >= -.425 && x < -.214 ? alaska : y >= .166 && y < .234 && x >= -.214 && x < -.115 ? hawaii : lower48).invert(coordinates$1);
	};
	...
	albersUsa.fitExtent = function(extent, object$1) {
		return fitExtent(albersUsa, extent, object$1);
	};
...
function azimuthalInvert(angle$1) {
	return function(x, y) {
		var z = sqrt(x * x + y * y), c = angle$1(z), sc = sin(c), cc = cos(c);
...
function mercatorProjection(project) {
	var m = projection(project), center = m.center, scale = m.scale, translate = m.translate, clipExtent = m.clipExtent, x0$5 = null, y0$5, x1$1, y1$1;
	m.scale = function(_) {
	...
	function reclip() {
		...
		return clipExtent(x0$5 == null ? [[t[0] - k, t[1] - k], [t[0] + k, t[1] + k]] : project === mercatorRaw ? [[Math.max(t[0] - k, x0$5), y0$5], [Math.min(t[0] + k, x1$1), y1$1]] : [[x0$5, Math.max(t[1] - k, y0$5)], [x1$1, Math.min(t[1] + k, y1$1)]]);
	}
	return reclip();
}
...
function conicConformalRaw(y0$5, y1$1) {
	var cy0 = cos(y0$5), n = y0$5 === y1$1 ? sin(y0$5) : log(cy0 / cos(y1$1)) / log(tany(y1$1) / tany(y0$5)), f = cy0 * pow(tany(y0$5), n) / n;
...
function conicEquidistantRaw(y0$5, y1$1) {
	var cy0 = cos(y0$5), n = y0$5 === y1$1 ? sin(y0$5) : (cy0 - cos(y1$1)) / (y1$1 - y0$5), g = cy0 / n + y0$5;
...
function identity_default() {
	var k = 1, tx = 0, ty = 0, sx = 1, sy = 1, alpha = 0, ca, sa, x0$5 = null, y0$5, x1$1, y1$1, kx = 1, ky = 1, transform = transformer({ point: function(x, y) {
		var p = projection$1([x, y]);
