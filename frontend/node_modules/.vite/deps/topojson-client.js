...
function feature_default(topology, o) {
	...
	return o.type === "GeometryCollection" ? {
		...
		features: o.geometries.map(function(o$1) {
			return feature(topology, o$1);
		})
	} : feature(topology, o);
}
...
function object(topology, o) {
	...
	function line(arcs$1) {
		var points = [];
		for (var i = 0, n = arcs$1.length; i < n; ++i) arc(arcs$1[i], points);
		if (points.length < 2) points.push(points[0]);
		return points;
	}
	function ring(arcs$1) {
		var points = line(arcs$1);
		while (points.length < 4) points.push(points[0]);
		return points;
	}
	function polygon(arcs$1) {
		return arcs$1.map(ring);
	}
	function geometry(o$1) {
		var type = o$1.type, coordinates;
		switch (type) {
			case "GeometryCollection": return {
				type,
				geometries: o$1.geometries.map(geometry)
			};
			case "Point":
				coordinates = point(o$1.coordinates);
				break;
			case "MultiPoint":
				coordinates = o$1.coordinates.map(point);
				break;
			case "LineString":
				coordinates = line(o$1.arcs);
				break;
			case "MultiLineString":
				coordinates = o$1.arcs.map(line);
				break;
			case "Polygon":
				coordinates = polygon(o$1.arcs);
				break;
			case "MultiPolygon":
				coordinates = o$1.arcs.map(polygon);
				break;
			default: return null;
		}
		return {
...
function stitch_default(topology, arcs) {
	...
	function flush(fragmentByEnd$1, fragmentByStart$1) {
		for (var k in fragmentByEnd$1) {
			var f = fragmentByEnd$1[k];
			delete fragmentByStart$1[f.start];
			delete f.start;
			delete f.end;
...
function extractArcs(topology, object$1, filter) {
	...
	function extract1(arcs$1) {
		arcs$1.forEach(extract0);
	}
	function extract2(arcs$1) {
		arcs$1.forEach(extract1);
	}
	function extract3(arcs$1) {
		arcs$1.forEach(extract2);
	}
	function geometry(o) {
...
function mergeArcs(topology, objects) {
	...
	polygons.forEach(function(polygon) {
		if (!polygon._) {
			...
			while (polygon = neighbors.pop()) {
				...
				polygon.forEach(function(ring) {
					ring.forEach(function(arc) {
						polygonsByArc[arc < 0 ? ~arc : arc].forEach(function(polygon$1) {
							if (!polygon$1._) {
								polygon$1._ = 1;
								neighbors.push(polygon$1);
							}
						});
					});
				});
	...
	return {
		type: "MultiPolygon",
		arcs: groups.map(function(polygons$1) {
			var arcs = [], n;
			polygons$1.forEach(function(polygon) {
				polygon.forEach(function(ring) {
					ring.forEach(function(arc) {
...
function neighbors_default(objects) {
	...
	function line(arcs, i$1) {
		arcs.forEach(function(a) {
			if (a < 0) a = ~a;
			var o = indexesByArc[a];
			if (o) o.push(i$1);
			else indexesByArc[a] = [i$1];
		});
	}
	function polygon(arcs, i$1) {
		arcs.forEach(function(arc) {
			line(arc, i$1);
		});
	}
	function geometry(o, i$1) {
		if (o.type === "GeometryCollection") o.geometries.forEach(function(o$1) {
			geometry(o$1, i$1);
		});
		else if (o.type in geometryType) geometryType[o.type](o.arcs, i$1);
	}
	var geometryType = {
		LineString: line,
		MultiLineString: polygon,
		Polygon: polygon,
		MultiPolygon: function(arcs, i$1) {
			arcs.forEach(function(arc) {
				polygon(arc, i$1);
			});
		}
...
function quantize_default(topology, transform) {
	...
	function quantizeArc(input) {
		var i = 0, j = 1, n$1 = input.length, p, output = new Array(n$1);
		output[0] = t(input[0], 0);
		while (++i < n$1) if ((p = t(input[i], i))[0] || p[1]) output[j++] = p;
		if (j === 1) output[j++] = [0, 0];
		output.length = j;
		return output;
	}
